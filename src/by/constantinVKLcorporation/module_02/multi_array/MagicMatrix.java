package by.constantinVKLcorporation.module_02.multi_array;

import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStreamReader;

/**
 * 16. Магическим квадратом порядка n называется квадратная матрица размера nxn, составленная из чисел 1, 2, 3,
 * ..., n*n так, что суммы по каждому столбцу, каждой строке и каждой из двух больших диагоналей равны между
 * собой. Построить такой квадрат. Пример магического квадрата порядка 3:
 */
public class MagicMatrix {
    public static void main(String[] args) throws IOException {

        //Сумма чисел в каждой строке, столбце и на диагоналях.
        // Называется магической константой , М. Магическая константа нормального
        // волшебного квадрата зависит только от n.
        /*Порядок n       M(n)

        3                 15
        4                 34
        5                 65
        6                 111
        7                 175
        8                 260
        9                 369
        10                505
        11                671
        12                870
        13                1105

        */
           BufferedReader reader = new BufferedReader(new InputStreamReader(System.in));

                int [][] magic;
                int n;

                System.out.println("Введите порядок магического квадрата");
                n = Integer.parseInt(reader.readLine());

                while (n < 2) {

                    System.out.println("Магический квадрат данного порядка невозможен, введите другое значение");
                    n = Integer.parseInt(reader.readLine());
                }
                if ((n - 1) % 2 == 0) {//первый случай для нечетного n
                    magic = unevenMagicSquare(n);
                } else if (n % 4 == 0) {//второй случай для кратных 4
                    magic = fourMagicSquare(n);
                } else { //третий случай
                    magic = evenOddMagicSquare(n);
                }
                printMatrix(magic);//вывод
        }

        private static int [][] unevenMagicSquare (int n){ //метод для первого случая
            int [][] magic = new int[n][n];
            int i = 0;
            int j = (n - 1) / 2;
            magic[i--][j++] = 1;
            for (int k = 2; k <= n * n; i--, j++, k++) {
                if (i < 0 && j > n - 1) {
                    i += 2;
                    j--;
                }
                if (i < 0) {
                    i = n - 1;
                }
                if (j > n - 1) {
                    j = 0;
                }
                if (magic[i][j] != 0) {
                    i += 2;
                    j--;
                }
                magic[i][j] = k;
            }
            return magic;
        }


        private static int [][] fourMagicSquare (int n){
            int [][] magic = new int[n][n];
            int temp;
            int k = 1;

            for (int i = 0; i < magic.length; i++) {
                for (int j = 0; j < magic.length; j++) {
                    magic[i][j] = k;
                    k++;
                }
            }

            for (int i = 0; i < n; i += 4 ) {
                for (int j = 1; j < n; j += 4) {
                    magic[i][j] = n * n + 1 - magic[i][j];
                    magic[i + 3][j] = n * n + 1 - magic[i + 3][j];
                    magic[i][j + 1] = n * n + 1 - magic[i][j + 1];
                    magic[i + 3][j + 1] = n * n + 1 - magic[i + 3][j + 1];
                }
            }
            for (int i = 1; i < n; i += 4 ) {
                for (int j = 0; j < n; j += 4) {
                    magic[i][j] = n * n + 1 - magic[i][j];
                    magic[i + 1][j] = n * n + 1 - magic[i + 1][j];
                    magic[i][j + 3] = n * n + 1 - magic[i][j + 3];
                    magic[i + 1][j + 3] = n * n + 1 - magic[i + 1][j + 3];
                }
            }
            return magic;
        }







        private static int [][] evenOddMagicSquare (int n){
            int [][] magic = new int[n][n];
            int [][] magicNNN = fourMagicSquare(n - 2);
            for (int i = 1; i < n - 1; i++) {
                for (int j = 1; j < n - 1; j++) {
                    magic[i][j] = magicNNN[i - 1][j - 1] + 2 * (n - 1);
                }
            }
            int m = n / 2;
            int d = n * n + 1;

            magic[0][0] = 3 * m - 1;
            magic[0][n - 1] = 1;
            magic[n - 1][0] = d - 1;
            magic[n - 1][n - 1] = d - 3 * m + 1;

            int i, j;
            for (i = 1; i <= m - 2; i++) {
                magic[0][i] = 2 * i + 1;
            }
            for (j = 1; j <= m; j++) {
                magic[0][j + i - 1] = d - 2 * j;
            }
            for (j = 1; j < n - 1; j++) {
                magic[n - 1][j] = n * n + 1 - magic[0][j];
            }
            magic[1][0] = 2 * m - 1;
            for (i = 1; i <= m / 2; i++) {
                magic[i + 1][0] = 3 * m - 1 - i;
            }
            i--;
            for (j = 1; j <= m / 2 + 1; j++) {
                magic[i + j + 1][0] = d - 4 * m + 1 + j;
            }
            j--;
            for (int q = 1; q <= m / 2 - 1; q++, i++){
                magic[i + j + q + 1][0] = 3 * m - 1 + q;
                magic[i + j + q + 2][0] = d - 2 * m - q;
            }

            for (i = 1; i < n - 1; i++) {
                magic[i][n - 1] = n * n + 1 - magic[i][0];
            }

            return magic;
        }

        private static void printMatrix (int [][] square) {
            for (int[] array : square) {
                for (int element : array) {
                    System.out.printf("%4d ", element);
                }
                System.out.println();
            }
        }
}
